...
def decode_jwt(token):
    try:
        ...
        return jsonify({"error": "Invalid signature"}), 401  # Invalid signature
    except jwt.ExpiredSignatureError:
        return jsonify({"error": "Token has expired"}), 401  # Token has expired
    except jwt.InvalidTokenError:
        return jsonify({"error": "Invalid token"}), 401  # Invalid token
